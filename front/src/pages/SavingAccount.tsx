...
function SavingCurrent() {
  ...
  const fetchAccount = () => {
    axios("http://localhost:8080/?action=getAccounts&account_type=savings")
      .then((response) => {
  ...
  const handleDelete = (id: number) => {
    axios
      .delete(`http://localhost:8080/?action=deleteAccount&account_id=${id}`)
      .then((response) => {
        fetchAccount();
        setAccounts((prevAccount) => prevAccount.filter((item) => item.id !== id));
    ...
    <div className="relative overflow-x-auto h-full w-full">
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {error && (
            <tr>
              <td className="text-red-600 text-center" colSpan={6}>
                {error}
              </td>
            </tr>
          )}
        ...
        <div className="fixed inset-0 bg-gray-500  bg-opacity-75 flex justify-center items-center z-50">
          <div className="bg-white p-6 w-[600px] rounded-lg shadow-lg ">
            ...
            <div>
              ...
              <input
                type="number"
                value={selectedAccount.interest_rate || ""}
                onChange={(e) => setSelectedAccount({ ...selectedAccount, interest_rate: parseFloat(e.target.value) })}
                className="w-full px-3 py-2 border border-gray-300 rounded-md mb-4"
              />
            </div>
            <div>
